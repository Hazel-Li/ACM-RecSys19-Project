...
class SessionDataset:
    ...
    def __init__(self, data, sep='\t', session_key='session_id', user_key='user_id',item_key='item_id', time_key='timestamp', n_samples=-1, itemmap=None, time_sort=False):
        """
        Args:
    ...
    def get_click_offsets(self):
        ...
        """
        #offsets = np.zeros(self.df[self.user_key,self.session_key].nunique() + , dtype=np.int32)
        # group & sort the df by session_key and get the offset values
        self.df.sort_values([self.user_key, self.session_key, self.time_key], inplace=True)
        self.df.reset_index(drop=True, inplace=True)
        offset_session=np.r_[0,self.df.groupby([self.user_key,self.session_key],sort=False).size().cumsum()[:-1]]
        user_indptr=np.r_[0,self.df.groupby(self.user_key,sort=False)[self.session_key].nunique().cumsum()[:-1]]

        return offset_session,user_indptr

    def order_session_idx(self):
        """ Order the session indices """
        if self.time_sort:
            # starting time for each sessions, sorted by session IDs
            sessions_start_time = self.df.groupby([self.user_key, self.session_key])[self.time_key].min().values
...
class SessionDataLoader:
    ...
    def __iter__(self):
        ...
        offset_session,user_indptr = self.dataset.click_offsets
        session_idx_arr = self.dataset.session_idx_arr

        n_users=len(user_indptr)
        iters = np.arange(self.batch_size)
        offset_users=offset_session[user_indptr]
        user_idx_arr=np.arange(n_users-1)

        user_maxiter = iters.max()
        session_iters=user_indptr[iters]
        start = offset_session[session_idx_arr[iters]]
        end = offset_session[session_idx_arr[iters] + 1]
        session_start = offset_session[session_iters]
        session_end = offset_session[session_iters + 1]
        ...
        while not finished:
            ...
            for idx in mask:
                user_maxiter  += 1
                if user_maxiter  >= len(offset_users) - 1:
                    finished = True
                    break
                # update the next starting/ending point
                iters[idx] = user_maxiter
                start[idx] = offset_users[user_maxiter]
                end[idx] = offset_users[user_maxiter+1]
                session_iters[idx] = user_indptr[user_maxiter]
                session_start[idx] = offset_session[session_iters[idx]]
                session_end[idx] = offset_session[session_iters[idx]+1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-path', type=str, default='rsc19_train.txt')
    parser.add_argument('--dev-path', type=str, default='rsc19_valid.txt')
    parser.add_argument('--test-path', type=str, default='rsc19_test.txt')
    parser.add_argument('--batch-size', type=str, default=512)
    args = parser.parse_args()

    args.train_data = pd.read_csv(args.train_path, sep='\t', dtype={'item_id': np.int64})
    args.dev_data   = pd.read_csv(args.dev_path,   sep='\t', dtype={'item_id': np.int64})
    args.test_data  = pd.read_csv(args.test_path,  sep='\t', dtype={'item_id': np.int64})
